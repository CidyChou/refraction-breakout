...
export const FIGHTER_TYPES = {
  ...
  laser:{id:'laser',name:'激光机',short:'LASER',icon:'━',accent:'#d6a7ff',desc:'单发轨迹清晰，命中次数高，适合长线折返。',evolves:[{id:'prismbeam',name:'棱镜光束',tag:'PRISM BEAM',desc:'更长寿命、更高命中次数。'},{id:'overdrive',name:'过载光束',tag:'OVERDRIVE',desc:'更强伤害与射速。'}]},
  ...
  support:{id:'support',name:'增幅装置',short:'FIELD',icon:'◇',accent:'#ffe37a',desc:'自身不攻击，子弹穿过它的场域时会获得 Buff。',evolves:[{id:'resonance',name:'灼热场域',tag:'FIRE FIELD',desc:'子弹穿过时叠加 BURN。'},{id:'command',name:'棱镜场域',tag:'PRISM FIELD',desc:'子弹穿过时增加可命中次数与寿命。'}]},
};

export function makeFighterMods(){return{
  hitCount:0,life:0,pierce:0,split:0,explosive:0,chain:0,homing:0,
...
export function fighterStats(f,_supportPower=0){
  ...
  if(f.type==='scatter')s={cd:1.15,damage:.48,shots:2+star,spread:.18,kind:'normal',hitCount:1+Math.floor(star/2),life:4.4+star*.35,trailScale:1,explosive:0,bulletSpeedScale:1};
  else if(f.type==='laser')s={cd:1.70,damage:.42,shots:1,spread:0,kind:'laser',hitCount:5+star*2,life:5.2+star*.4,trailScale:1.15,explosive:0,bulletSpeedScale:1.2};
  else if(f.type==='pierce')s={cd:1.28,damage:.78,shots:star>=4?2:1,spread:star>=4?.04:0,kind:'pierce',hitCount:4+star*2,life:5.0+star*.45,trailScale:1.05,explosive:0,bulletSpeedScale:1.05};
  else if(f.type==='aoe')s={cd:1.62,damage:.60,shots:1,spread:0,kind:'aoe',hitCount:2+Math.floor(star/2),life:4.8+star*.3,trailScale:1.05,explosive:1+(star-1)*.45,bulletSpeedScale:.85};
  else s={cd:999,damage:0,shots:0,spread:0,kind:'support',hitCount:0,life:0,trailScale:1,explosive:0,bulletSpeedScale:1};
  ...
  if(f.evolution==='prism'){s.shots+=1;s.spread=.15;s.hitCount+=1;s.life+=.8}
  if(f.evolution==='storm'){s.shots+=4;s.damage*=.76;s.cd*=.82;s.spread=.20}
  if(f.evolution==='prismbeam'){s.hitCount+=3;s.life+=1.4;s.trailScale=1.45;s.damage*=1.08}
  if(f.evolution==='overdrive'){s.cd*=.72;s.damage*=1.36;s.hitCount+=1;s.trailScale=1.2}
  if(f.evolution==='railgun'){s.cd*=1.15;s.damage*=1.58;s.hitCount+=5;s.life+=1.1;s.trailScale=1.2}
