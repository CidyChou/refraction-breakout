...
export class Renderer{
  constructor(canvas,state){this.canvas=canvas;this.ctx=canvas.getContext('2d',{alpha:false});this.state=state;this.dpr=Math.min(PERFORMANCE.maxDpr,devicePixelRatio||1);this.staticLayer=this.buildStaticLayer();this.resize();addEventListener('resize',()=>this.resize())}
  buildStaticLayer(){
    const c=document.createElement('canvas');c.width=VIEW.width;c.height=VIEW.height;const ctx=c.getContext('2d',{alpha:false});
  ...
  background(){
    const {ctx,state:s}=this;ctx.drawImage(this.staticLayer,0,0,VIEW.width,VIEW.height);
    const quality=s.perf?.quality||'high',step=quality==='low'?2:1;
    ctx.save();ctx.fillStyle='#d9e8ff';for(let i=0;i<s.stars.length;i+=step){const st=s.stars[i];ctx.globalAlpha=st.a;ctx.fillRect(st.x,st.y,st.s,st.s*2)}ctx.restore();
  }
  ...
  bullet(b){
    ...
    if(quality==='high'&&s.bullets.length<PERFORMANCE.highLoadBulletLabels&&((b.slowBuff||0)||(b.burnBuff||0)||(b.prismBuff||0))){ctx.shadowBlur=0;ctx.fillStyle=c;ctx.font='700 8px sans-serif';ctx.textAlign='center';const parts=[];if(b.slowBuff)parts.push(`S${b.slowBuff}`);if(b.burnBuff)parts.push(`B${b.burnBuff}`);if(b.prismBuff)parts.push(`P${b.prismBuff}`);ctx.fillText(parts.join('·'),b.x,b.y-b.r-7)}ctx.restore();
  }


  enemyColor(t){return enemyType(t).color}
  ...
  enemy(e){
    const {ctx,state:s}=this,c=this.enemyColor(e.type),phase=e.phase||0,quality=s.perf?.quality||'high';
    ...
    if(quality!=='low'&&['spinner','warden','pulse','shield','tank','regen','carrier','titan','bumper'].includes(e.type)){
    ...
    ctx.scale(sx,sy);ctx.shadowBlur=quality==='high'?14:0;ctx.shadowColor=c;ctx.fillStyle='rgba(18,22,33,.94)';ctx.strokeStyle=c;ctx.lineWidth=e.type==='titan'?2.4:2;
    ...
    }else if(e.type==='magnet'&&quality!=='low'){
    ...
    if(e.spawnT<=0&&(quality!=='low'||e.hp<e.maxHp||e.type==='titan')){ctx.save();ctx.fillStyle='rgba(11,18,28,.78)';ctx.fillRect(e.x-e.r,e.y+e.r+8,e.r*2,5);ctx.fillStyle='rgba(255,255,255,.12)';ctx.fillRect(e.x-e.r,e.y+e.r+8,e.r*2,5);ctx.fillStyle=c;ctx.fillRect(e.x-e.r,e.y+e.r+8,e.r*2*Math.max(0,e.hp/e.maxHp),5);ctx.restore()}
  }

  fx(){
    const {ctx,state:s}=this,quality=s.perf?.quality||'high',particleStep=quality==='low'?2:1;
    for(let i=0;i<s.particles.length;i+=particleStep){const p=s.particles[i],a=Math.max(0,p.life/(p.max||p.life||1));ctx.globalAlpha=a;ctx.fillStyle=p.kind==='burnfx'?'#ffb46b':'rgba(234,243,255,.88)';ctx.beginPath();ctx.arc(p.x,p.y,p.size||2,0,TAU);ctx.fill()}ctx.globalAlpha=1;
    ...
    if(quality!=='low')for(const f of s.floating){ctx.save();ctx.globalAlpha=Math.max(0,f.life/.8);ctx.fillStyle=f.color||'#fff';ctx.font='700 12px sans-serif';ctx.textAlign='center';ctx.fillText(f.text,f.x,f.y);ctx.restore()}
  }
